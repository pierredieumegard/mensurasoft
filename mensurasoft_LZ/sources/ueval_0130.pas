//unité ueval_0130.pas : partie du logiciel Mensurasoft_lz
//évaluation des fonctions de transformation
...
transformé en unité pour Lazarus par Pierre Dieumegard (pierre.dieumegard@free.fr)
}
      ...
      'T' : begin
       ...
       'I' : begin
              if next3='NT(' then
                 begin
                 inc(i,3);
                 temp:=temp+fstr(int(domuls(dopowers(dofuncs(getbrackets(cal, i))))));
                 end;
              end;
      'A' : begin
...
function evaluechaine(ch:string):double;
begin
if clean(ch) then evaluechaine:=domuls(dopowers(dofuncs(ch)))
             else evaluechaine:=-777;
end;
