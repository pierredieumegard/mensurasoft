...
library bibcapt2_lazarus_calcapture;
...
uses
 ...
 // windows,
  Interfaces, controls,   forms, lcltype,
  Dialogs, fenetre_dialogue_bibcapt2_lazarus
...
//const mryes=6;   const mrno=7;
  var chloc:string;
      reponse:integer;
begin
  ShowMessage('Fenêtre fictive de réglage de l''interface');  //correct
  //If QuestionDlg('Attention','Data already exists. What to do', mtWarning,[mrYes,'Overwrite',mrNo,'Keep existing data'],'')=mrYes then ;
//chloc:=InputBox('Get some text input','Please type in some information', 'Some sample text');
//reponse:=MessageDlg('This in an information dialogue!', mtInformation, [mbOK], 0);
//QuestionDlg('test','more test', mtConfirmation,[mrYes,mrNo],'');
//if InputQuery ('Question', 'Type in some data', TRUE, chloc)    then ;    //plante
//form1.show;
ccalcapture1:=pchar('Fini !');
end;
function stdcalcapture1(ch:pchar):pchar;stdcall;export;alias:'stdcalcapture1';
begin stdcalcapture1:=ccalcapture1(ch);end;

function cfixcapture1(ch:pchar):longint; cdecl;export;alias:'cfixcapture1';
begin if string(ch)<>'XXX' then result:=0 else result:=-1;end;

function stdfixcapture1(ch:pchar):longint ; stdcall;export;alias : 'stdfixcapture1';
begin result:=cfixcapture1(ch);end;
...
stdcalcapture1,ccalcapture1,  stdfixcapture1,cfixcapture1,
